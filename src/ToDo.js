    ...
    if (this.state.isEditingTodo) {
      returnResult = (
        <form onSubmit={this.handleUpdate}>
          <label htmlFor="toDoItem">Editing ToDo</label>
          <input
            name="toDoItem"
            id="toDoItem"
            type="text"
      ...
      returnResult = (
        <div className="ToDo">
          <div>
            <li
              className={this.props.isDone ? "ToDo-Completed" : ""}
              onMouseDown={this.props.toggleToDo}
            >
              {this.props.item}
            </li>
            <button onClick={this.props.remove}>X</button>
            <button onClick={this.toggleToDoEdit}>Edit</button>
          </div>
        </div>
      );
    }
